...
class TransactionService {
    ...
    func deleteTransaction(transaction: Transaction) {
        // how do we find the transaction in the database? In web, we did it by ID.
        // That seems to make sense here as well, it would require adding an ID field to the Transction struct
    }
    ...
    func updateTransaction(transaction: Transaction) {
        ...
        let managedContext = appDelegate.persistentContainer.viewContext
        let fetchRequest = NSFetchRequest<NSManagedObject>(entityName: "Transaction")
        // predicate to match transaction on id
        let prediate = NSPredicate(format: "id = %@", transaction.objectID)
        fetchRequest.predicate = prediate
        var foundTransaction: [Transaction]
        do {
            foundTransaction = try managedContext.fetch(fetchRequest) as! [Transaction]
            print("Got \(transactions.count) transactions")
